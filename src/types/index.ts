...
export interface Conversation {
  ...
  channel?: 'private_phone' | 'group_phone' | 'scene'
  sceneLocationId?: string
  sceneWorldStep?: number
...
export interface ContactMemory {
  ...
  lastUsedAt?: number
  usageCount: number
  status?: 'active' | 'archived' | 'superseded' | 'summarized'
  pinned?: boolean
  sourceEventIds?: string[]
}

// ---- ChatSLG authoritative world model ----
export type TimeSlot = 'morning' | 'day' | 'evening' | 'night'
export type LocationAccess = 'public' | 'restricted' | 'private'
export type Audibility = 'clear' | 'muffled' | 'none'
export type SchedulePriority = 'base' | 'override' | 'commitment'
...
export type CompactLocationNode = Pick<LocationNode, 'id' | 'parentId' | 'name' | 'kind' | 'description' | 'access'>
...
export type OutfitPart = 'head' | 'top' | 'bottom' | 'outerwear' | 'footwear' | 'accessories'

export interface OutfitChangeProposal {
  characterId: string
  worldVersion: number
  patch: Partial<Record<OutfitPart, string>>
  reasonType: 'conversation_event' | 'schedule_change'
  sourceEventIds: string[]
...
export interface PendingPhoneMessage {
  id: string
  conversationId: string
...
// Deliberately a smaller text-only protocol than 1:1.
...
export type GroupAiBubble = GroupAiBubbleText
