...
export function ChatPage({ conversationIdOverride, embedded = false }: { conversationIdOverride?: string; embedded?: boolean } = {}) {
  ...
  if (conversation === null) {
    return (
      <div className={`flex ${embedded ? 'h-full' : 'h-[var(--app-height)]'} flex-col overflow-hidden bg-[#ededed]`}>
        <TopBar title="对话" showBack={!embedded} />
  ...
  if (isGroupConv) {
    ...
    if (group === null) {
      return (
        <div className={`flex ${embedded ? 'h-full' : 'h-[var(--app-height)]'} flex-col overflow-hidden bg-[#ededed]`}>
          <TopBar title="群聊" showBack={!embedded} />
  ...
  return (
    <div className={`relative flex ${embedded ? 'h-full' : 'h-[var(--app-height)]'} flex-col overflow-hidden bg-[#ededed]`}>
      <TopBar
        title={selectingMessages ? `已选择 ${selectedMessageIds.length} 条` : visibleHeaderTitle}
        showBack={!embedded && !selectingMessages}
