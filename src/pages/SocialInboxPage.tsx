...
export function SocialInboxPage() {
  ...
  return <div className="flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]"><TopBar title="互动收件箱" showBack /><div className="flex-1 overflow-y-auto p-3">{items.length === 0 ? <p className="py-10 text-center text-sm text-gray-400">还没有新的互动。</p> : <div className="space-y-2">{items.map((event) => { const [icon, color] = style(event.type); return <button key={event.id} type="button" onClick={() => event.momentId ? navigate(`/phone/moments?focus=${event.momentId}`) : event.groupId ? navigate(`/group/${event.groupId}`) : event.conversationId ? navigate(`/chat/${event.conversationId}`) : undefined} className={`flex w-full gap-3 rounded-xl bg-white p-3 text-left ${event.createdAt > lastReadAt ? 'ring-1 ring-[#07c160]/30' : ''}`}><span className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-full ${color}`}>{icon}</span><span><p className="text-sm text-gray-800">{event.summary}</p><p className="mt-1 text-[11px] text-gray-400">{new Date(event.createdAt).toLocaleString()}</p></span></button> })}</div>}</div></div>
}
