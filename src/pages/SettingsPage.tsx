  ...
  return (
    <div className="flex h-[var(--app-height)] flex-col overflow-hidden bg-[#f4f4f6]">
      ...
      <section className="mt-3 bg-white">
        <button onClick={() => navigate('/settings/scene-archives')} className="flex w-full items-center justify-between px-4 py-3.5 text-left">
      ...
      <section className="mt-3 bg-white px-4 py-3">
        ...
        <label className="mb-1 block text-xs text-gray-500">多功能模型（结构转换、记忆整理、世界观草稿等辅助任务，独立于主聊天模型）</label>
        <div className="mb-1 flex gap-2">
          {models.length > 0 ? (
            <select
      ...
      <section className="mt-3 flex-1 bg-white px-4 py-3">
        ...
        />
        <p className="mt-2 text-[11px] text-gray-400">
          这里只控制所有AI共通的说话语气和习惯 每个AI各自的人物设定在联系人名片里单独编辑 消息输出格式与小程序调用规则由系统固定处理 不在这里展示
        </p>
      </section>
      ...
      <section className="mt-3 bg-white px-4 py-3">
        ...
        <p className="mt-2 text-[11px] leading-relaxed text-gray-400">
          备份包含联系人、人设、聊天记录、朋友圈、仓库、世界书、地图和当前设置。设置里如果保存过 API Key，备份文件里也会带上，请不要发给别人。
        </p>
