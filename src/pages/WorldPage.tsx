...
import { weatherForWorld } from '../lib/worldWeather'
...
export function WorldPage() {
  ...
  const weather = weatherForWorld(map?.seed ?? world?.worldId ?? 'default-world', world?.day ?? 1, world?.slot ?? 'morning')
  ...
  return <div className="flex h-full min-h-0 flex-col bg-[#f4f4f6]">
    <header className="shrink-0 bg-white px-4 pb-2 pt-[calc(env(safe-area-inset-top)+10px)] shadow-sm"><div className="flex items-start justify-between gap-3"><div><h1 className="text-lg font-semibold">地点</h1><p className="text-xs text-gray-400">{formatWorldDate(world?.day ?? 1)} · {slotLabel(world?.slot ?? 'morning')} · {String(world?.hour ?? 8).padStart(2, '0')}:00</p></div><div className="rounded-xl bg-sky-50 px-2.5 py-1.5 text-right"><p className="text-xs font-medium text-gray-700">{weather.icon} {weather.label}</p><p className="text-[10px] text-gray-400">体感{weather.temperature}</p></div></div><div className="mt-2 flex items-center justify-between"><p className="text-[11px] text-gray-400">拖动地图 · 双指/滚轮缩放 · 点击建筑进入</p><button onClick={() => void expandWithAi()} disabled={expanding} className="text-xs text-violet-600">{expanding ? '生成中…' : 'AI 扩展'}</button></div><div className="mt-2 flex flex-wrap gap-x-2 gap-y-1">{Object.entries(TERRAIN_COLORS).map(([terrain, color]) => <span key={terrain} className="flex items-center gap-1 text-[9px] text-gray-500"><i className="h-2.5 w-2.5 rounded-sm" style={{ backgroundColor: color }} />{TERRAIN_LABELS[terrain as TerrainType]}</span>)}</div>{error && <p className="mt-2 rounded-lg bg-red-50 px-3 py-1.5 text-xs text-red-600">{error}</p>}</header>
