...
export function SearchOverlay({ onClose }: SearchOverlayProps) {
  ...
  const messages = useLiveQuery(async () => {
    const phone = await db.conversations.filter((conversation) => conversation.channel === 'private_phone' || conversation.channel === 'group_phone' || (!conversation.channel && !!(conversation.contactId || conversation.groupId))).primaryKeys()
    const ids = new Set(phone)
    return (await db.messages.toArray()).filter((message) => ids.has(message.conversationId))
  }, []) ?? EMPTY_ARRAY
