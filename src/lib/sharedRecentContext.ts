...
export async function recentSharedOriginalContext(
  ...
  // Phone history is visible only to its actual participant(s). Scene
  // transcripts are deliberately excluded here: scene continuity is injected
  ...
  const conversationIds = new Set(conversations.filter((conversation) => {
    ...
    if (conversation.groupId) {
      ...
      return !!group && group.memberContactIds.some((id) => ids.has(id))
    }
    return false
