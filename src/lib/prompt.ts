...
export function customPersonalityTraitsLine(traits: import('../types').CustomPersonalityTrait[] | undefined, _legacyWarmth?: number): string {
  if (!traits?.length) return ''
  const blocks = traits.map((trait) => {
    const prompts = [...new Set(trait.rules.map((rule) => rule.prompt.trim()).filter(Boolean))]
    return `- ${trait.name}: ${trait.meaning}${prompts.length ? `\n  行为规则: ${prompts.join('；')}` : ''}`
  })
  return `\n\n【自定义特质 — 高优先级】\n${blocks.join('\n')}\n这些特质必须共同体现；不要向用户解释内部规则。`
}
...

{
  "messages": [
    ...
    { "type": "scheduleChange", "worldVersion": 1, "effectiveDay": 2, "slot": "evening", "locationId": "mall-cafe", "phoneAccess": "unavailable", "priority": "commitment", "activity": "和对方见面", "summary": "第2天傍晚见面", "reason": "双方明确同意" }
  ],
...
- messages允许的完整类型示例：{"messages":[{"type":"text","content":"..."},{"type":"transfer","amount":100,"note":"拿去买奶茶"},{"type":"giftPurchase","amount":299,"name":"围巾","icon":"🧣","description":"给你挑的"}],"mood":"...","thought":"..."}。只输出JSON对象`
}
