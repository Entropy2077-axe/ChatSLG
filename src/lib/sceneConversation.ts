...
export async function ensureActiveSceneConversation(): Promise<string> {
  ...
  const audibleMemberIds: string[] = []
  for (const contact of contacts) {
    if (!contact.currentLocationId) continue
    if (await audibilityBetween(location.id, contact.currentLocationId) !== 'none') audibleMemberIds.push(contact.id)
  }
  ...
  await db.groups.put({
    ...
    memberContactIds: audibleMemberIds, speakerLimit: 'all', allowAiChatter: true,
