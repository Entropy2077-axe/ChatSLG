...
- 测试日期：2026-07-17T08:09:16.837Z
- 代码/应用版本：0.1.33 / 3b9019b
- 主模型：deepseek-v4-pro
- Utility 模型：deepseek-v4-flash
- 测试数据库：chatslg-ai-eval-cli-1784275658594（隔离=是）
...
| acceptance-e2e-location-colloquial | real | model_missed | location状态结果 |
| acceptance-e2e-photo-negative | real | model_missed | 主模型重写后仍未通过硬性验收：客观逻辑错误：用户只是描述照片中的外套，没有请求任何动作；草稿却把‘旧照片’误解为用户声称角色发过照片，并质疑‘记错人’，属于混淆描述与事实主张。 |
| acceptance-e2e-multi-state-colloquial | real | model_missed | 主模型重写后仍未通过硬性验收：客观逻辑错误：用户明确要求‘这礼拜都戴着蝴蝶结’，草稿只回应‘我没蝴蝶结’就跳过该请求，未按人设‘面对合理明确请求会自然答应并照做’处理；应明确表示接受或给出可行替代方案，而非仅否定后忽略。 |
| acceptance-e2e-multi-state-colloquial | real | model_missed | location状态结果 |
| acceptance-e2e-multi-state-colloquial | real | model_missed | location状态结果 |
