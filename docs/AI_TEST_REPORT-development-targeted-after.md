# ChatSLG AI 回归测试报告

> 本报告只陈述当前样本中观察到的结果。样本量不足时不据此宣称达到 99%。

## 测试信息

- 测试日期：2026-07-17T08:42:08.137Z
- 代码/应用版本：0.1.33 / 5234f23
- 主模型：deepseek-v4-pro
- Utility 模型：deepseek-v4-flash
- 测试数据库：chatslg-ai-eval-cli-1784277718158（隔离=是）
- 随机种子：模型接口未提供固定随机种子；重复运行用于观察波动。
- 核心聊天逻辑是否修改：是

## 总体指标

| 指标 | 数值 |
| --- | ---: |
| 总运行次数 | 2 |
| 实际执行次数 | 2 |
| 因配置阻塞次数 | 0 |
| 真实模型运行次数 | 2 |
| 真实模型完全通过 | 2 |
| 真实模型首次直接通过率 | 50.0% |
| 真实模型修复后最终通过率 | 100.0% |
| 发生修复/重试的比例 | 50.0% |
| 修复/重试救回率 | 100.0% |
| 完整端到端通过率 | 0.0% |
| 直接裁决器通过率 | 100.0% |
| 独立验收集通过率 | 0.0% |
| 开发回归集通过率 | 100.0% |
| 实际故障注入通过率 | 0.0% |
| 分类表自检次数（不计生产通过率） | 0 |
| Mock/分类自检运行次数 | 0 |
| 回复格式成功率 | 100.0% |
| 热闹度条数达标率 | 0.0% |
| 多人参与率 | 0.0% |
| 衣着状态召回率 | 0.0% |
| 衣着误触发率 | 0.0% |
| 日程状态召回率 | 0.0% |
| 日程误触发率 | 0.0% |
| 地点状态召回率 | 0.0% |
| 地点误触发率 | 0.0% |
| 多状态同时提交成功率 | 100.0% |
| 数据库提交成功率 | 100.0% |
| 平均耗时 | 4360 ms |
| P50 / P95 | 3160 ms / 5560 ms |
| 平均模型调用次数 | 1.50 |



## 分类成功率

| 分类 | 运行 | 通过 | 成功率 |
| --- | ---: | ---: | ---: |
| location | 1 | 1 | 100.0% |
| multi_state | 1 | 1 | 100.0% |

## 场景列表

| 场景ID | 分类 | 测试集 | 覆盖层级 | 模式 | 默认重复 | 说明 |
| --- | --- | --- | --- | --- | ---: | --- |
| location-wish-negative | location | 开发回归 | adjudicator_only | 真实模型 | 1 | 只有想去但没有行动，不移动。 |
| multi-state-split-bubbles | multi_state | 开发回归 | adjudicator_only | 真实模型 | 1 | 立即移动的明确承接出现在前一条气泡时，后续衣着和日程气泡不得让地点提交漏判。 |

## 失败案例摘要

| 场景 | 模式 | 失败类型 | 原因 |
| --- | --- | --- | --- |
| — | — | — | — |


## 已确认的代码问题

- 当前样本未复现新的代码问题。

## 尚不能确认的问题

- 无。

## 结果解读

“首次直接通过率”表示没有任何传输重试、结构修复或状态重写就成功；“修复后最终通过率”才包含被重试机制救回的结果。完整端到端状态场景必须先调用真实聊天引擎产生角色回复，再检查状态裁决和数据库；直接裁决器场景使用脚本化证据，只证明裁决器本身。分类表自检不是故障注入，不计入生产正确率。即使固定样本全部通过，也不能外推为未覆盖输入达到 99% 可靠性。

## 后续对比方法

保持场景ID、模型、数据库隔离方式和重复次数不变，从“天眼 → AI测试台”运行全部场景。导出JSON后按 scenarioId + repetition 比较硬断言、失败类型、数据库最终状态、模型调用数和P50/P95耗时。Mock结果不得与真实模型成功率混算。
