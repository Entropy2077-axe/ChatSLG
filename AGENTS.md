...
`buildSystemPrompt()` 拼接顺序：`stylePrompt + 人物设定 + 记忆 + 实时上下文(架空世界时间/用户资料/最近事件) + 固定协议`。
...
- `Contact.upcomingPlans?: PlanItem[]`，新约定使用`PlanItem{id,text,worldDay?,worldSlot?,createdAt}`。`worldDay`是世界连续日，`worldSlot`只能是早晨/白天/傍晚/夜晚；模型能从架空历当前时点结合“明天晚上”之类相对表达推算就填，算不出来就留空。旧存档的`date?: YYYY-MM-DD`字段仅为兼容保留，不再展示给AI。
- **提取时机和现有记忆更新合并成同一次调用**，不单独起一次API请求：`buildMemoryUpdatePrompt`要求模型顺带输出`"plans":[{"text":"...","worldDay":12,"worldSlot":"evening"}]`（只要这批新消息里新出现的约定，不重复已有的），和记忆、关系一起解析落库。
- `activeWorldPlans()`/`activeWorldPlansText()`按世界日和四时段过期过滤（没有明确时点的计划靠`MAX_UPCOMING_PLANS=8`硬上限兜底），并注入私聊/群聊Prompt。旧的现实日期过滤函数已删除。
- `ContactCardPage`记忆区块顺带展示了当前有效的约定列表，`hasMemory`判断也把它算进去了。
- **明确没做的**：不会主动推送提醒（没有闹钟/通知机制），也不会理解"取消/改期"这种口头撤销——纯粹是世界日到期后不再注入，靠模型自己看架空时间判断要不要提。
...
## 系统提示词里的架空时间上下文注入
`src/lib/worldCalendar.ts`以`world.day`为唯一日期基准：每季30天、春夏秋冬四季、一年120天；`world.slot`仍是早晨/白天/傍晚/夜晚。私聊、群聊、剧情大纲、记忆整理、世界回合和管理员Prompt预览统一调用`modelWorldTimeText()`，AI不得看到或推测现实年月日、星期、设备分钟。`createdAt`/`Date.now()`仍用于数据库排序、冷却、日志和玩家UI展示，不能全局替换。跨场景原文只给递增记录编号；旧群聊摘要的现实时间前缀在注入模型前剥离。`chatEngine.ts`的`runAiTurn`每次仍会现算用户资料与`contact.pendingEvents`（读取后立即清空）。

## 世界时间第二阶段：约定、工资与生活模拟
- **约定/日程**：正式`appointments`、软性`PlanItem`、群共同计划和`characterSchedules`统一使用`worldDay + worldSlot`。四时段内按早晨→白天→傍晚→夜晚判断到期；世界周只是`(world.day - 1) % 7`的架空循环，不对应现实星期。
- **工资**：`settleSalaries(world.day)`按已过去的世界日结算，月薪按每个世界月30日折算。`lastSalaryWorldDay`是唯一运行时游标；已有旧存档首次进入新结算制时从当前世界日开始，不追补现实日期工资。每个世界回合都会调用，但同一世界日幂等。
- **生活模拟**：`settleLifeSimulationForWorldTurn()`只消费世界引擎已验证的`characterDiaries`，按`worldStep`幂等同步`contactLifeStates/lifeEvents`；不再读取离线时长、设备小时或现实日期，也不再另起AI调用补写生活。生活事件按世界日过期。
- `createdAt`、`occurredAt`、冷却时间、API自动调用日限额等仍允许使用现实时间，因为它们只负责排序、日志和成本控制，不能写进AI可见的世界语义。

## 世界时间第三阶段：确定性天气与季节规则
`src/lib/worldWeather.ts`是唯一天气规则源。天气不存随机结果，也不调用AI，而是由`worldMaps.active.seed + world.day + world.slot`纯函数推导；同一世界种子、同一世界日和时段在读档后一定得到相同结果。每3个世界日共享一个主天气锋面，并允许代码确定的少量逐日变化。
- 春夏秋冬各有独立天气权重；降雪只在冬季，雷雨只会出现在春夏等合理季节。一天有一个主天气，阵雨、雷雨、雾等按四时段确定性地出现/消散，不能每次渲染重新随机。
- `WorldWeatherSnapshot`包含天气类型、玩家标签、图标、体感、户外规则、衣着规则和少量季节背景。模型只看到架空季节与四时段天气，不看到现实天气、设备位置或现实钟点。
- `buildLogicContext`、世界回合、朋友圈生成、联系人Prompt预览统一注入同一份天气硬状态。AI不得擅自换天气、凭天气取消明确约定，或让所有角色机械谈天气。
- 世界回合日志同步到生活模拟时，会把确定天气写入`LifeEvent/ContactLifeState.weatherLabel`。雷雨或大雪中的明确户外活动会有少量额外精力/压力影响；不做疾病、灾害、封路和逐地区小气候。
- 玩家在时间页和地图页可看到当前天气。当前第一版采用全世界统一天气；后续若做地区气候，必须仍以同一世界种子确定性推导，不能交给AI随机决定。

## 世界时间第四阶段：稀疏高质量事件导演
`src/lib/eventDirector.ts`统一决定世界中的“值得记录的小事件”。代码先决定是否发生、类型、地点和参与者，只有选中候选后才调用一次多功能模型把硬前提润色成35–100字摘要；模型无权决定事件是否发生。
- 候选优先级：季节首日的公共换季迹象 → 正在成行的已确认约会小插曲 → 通过确定性门槛的雷雨/大雪环境事件 → 同地点且已有关系链的两人日常小插曲。每个世界时段最多一条。
- 普通事件共享至少12世界步（3世界日）冷却；社交事件至少20步，同一参与者组合120步（30世界日）内不重复。概率门槛来自世界种子、世界日和四时段，读档不会重新掷骰。
- 原`worldTurn.ts`每回合按同地点额外生成最多4条`encounters`的高频路径已删除。没有导演事件时必须保持普通生活，主模型不得自行添加突发事件。
- 润色器禁止新增角色/地点以及伤亡、灾害、犯罪、失踪、财产损失、停电封路、关系突变、重大决定和新长期承诺；代码还会对这些结果做关键词拒绝，并拒绝摘要点名未授权角色。润色失败就不发生事件，不使用低质量兜底文案。
- 事件在主世界回合生成角色日志之前确定。场景事件参与者必须被主回合放在事件地点，否则整轮重写；事件、`PerceivedEvent`和角色日志在同一事务落库，参与者日志的`sourceEventIds`包含该事件。
- `WorldEvent`新增`seasonal/weather/story`类型及`worldDay/worldSlot/directedEventKey/importance`兼容字段。公共事件会显示在时间页；场景事件只有玩家本人是参与者时才显示，不能泄漏其他角色的私下事件。
...
## 世界日程系统（`characterSchedules` + `lib/schedule.ts`）
角色作息的唯一权威数据是`CharacterSchedule`：`dayOfWeek`表示由世界连续日推导的0–6架空循环，`effectiveDay`表示一次性的世界日，`slot`只能是四时段。`commitment > override > base`；地点必须是世界地图里的叶子ID。`lib/schedule.ts`只读取这套世界状态，绝不能重新读取现实小时、现实星期或`Contact.schedule`。
- 人设生成同一次调用输出`worldSchedule`，新联系人只写`characterSchedules`；旧`Contact.schedule`/`scheduleOverrides`及其现实日期字段仅为旧存档兼容，不再参与运行时判断，也不展示给AI。
- 联系人名片显示世界周第N日/世界第N天和四时段；聊天、群聊、朋友圈、状态裁决统一使用权威世界日程。用户主动发起聊天仍即时回复，不用作息人为延迟API。
- 正式约会写入`appointments`（世界日+四时段），轻量口头约定写入`PlanItem`（世界日+四时段）。群共同计划的新字段同样是`worldDay/worldSlot`，`scheduledAt`只保留旧存档兼容。
...
- `src/lib/`：**chatEngine.ts(核心！sendMessage+triggerAiTurn+后台引擎，1:1专用)** / **groupChatEngine.ts(群聊版后台引擎，复用chatEngine.ts的store)** / groupChat.ts(群聊发言人选择+多人设prompt+协议解析+名字前缀leak清洗) / worldTurn.ts(推进四时段并触发工资与生活结算) / worldCalendar.ts(架空历与模型时间文本) / lifeSimulation.ts(世界日志→生活状态) / finance.ts(世界日工资与钱包) / schedule.ts(仅按世界日/四时段解析权威日程) / proactiveChat.ts(AI主动找你聊天) / webSearch.ts(Tavily搜索) / knowledgeBase.ts(知识库：关键词触发+去重+手动搜索) / photoSearch.ts(Pexels搜图+waifu.pics随机动漫图+console日志) / avatarCategory.ts(头像分类：性格标签加权随机，代码决定不是LLM) / consoleCapture.ts(全局console捕获) / unread.ts(未读消息计数) / updateCheck.ts(GitHub Release版本检查) / deepseek.ts(jsonMode开关+角色合并) / aiProtocol.ts(解析+兜底+委托reward clamp+scheduleChange解析+leak修复+平衡括号JSON提取) / prompt.ts(三层提示词+人设生成+世界日程+世界观草稿+头像搜图关键词) / memory.ts(记忆+关系增量+群聊记忆+世界约定提取) / relationship.ts(用户-AI五维度) / contactRelations.ts(AI-AI关系标签+情感分类) / moments.ts(朋友圈生成引擎+配图+级联删除) / shop.ts(独立商品生成) / wallet.ts(货币常量) / messagePreview.ts / randomTraits.ts / contact.ts(displayName) / image.ts(图片压缩) / search.ts / time.ts(仅玩家UI与运维日期工具) / colors.ts / avatarEmojis.ts。
